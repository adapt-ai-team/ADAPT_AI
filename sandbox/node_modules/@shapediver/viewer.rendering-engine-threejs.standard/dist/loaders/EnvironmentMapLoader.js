...
class EnvironmentMapLoader {
    ...
    notify(eventId, failed = false) {
        let event;
        if (failed) {
            event = { type: viewer_shared_types_1.TASKTYPE.ENVIRONMENT_MAP_LOADING, id: eventId, progress: 1, status: `Loading of EnvironmentMap failed` };
            this._eventEngine.emitEvent(viewer_shared_services_1.EVENTTYPE.TASK.TASK_CANCEL, event);
        }
        else {
            event = { type: viewer_shared_types_1.TASKTYPE.ENVIRONMENT_MAP_LOADING, id: eventId, progress: 1, status: `Loaded EnvironmentMap` };
            this._stateEngine.viewers[this._renderingEngine.id].environmentMapLoaded.resolve(true);
            this._stateEngine.viewers[this._renderingEngine.id].environmentMapLoaded = new viewer_shared_services_1.StatePromise();
    ...
    load(name) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const event = { type: viewer_shared_types_1.TASKTYPE.ENVIRONMENT_MAP_LOADING, id: eventId, data: { input: name }, progress: 0, status: `Loading EnvironmentMap` };
            ...
            if (!Array.isArray(name) && (name.startsWith('["https') && name.endsWith('"]')))
                try {
                    ...
                    throw this._logger.handleError(viewer_shared_services_1.LOGGINGTOPIC.VIEWER, `EnvironmentMapLoader.load`, error);
                }
            ...
            try {
                ...
                this.notify(eventId, true);
                throw this._logger.handleError(viewer_shared_services_1.LOGGINGTOPIC.VIEWER, `EnvironmentMapLoader.load`, e);
            }
        });
    }
