...
class MaterialLoader {
    ...
    assignEnvironmentMap(e, type) {
        ...
        for (let m in this._materialCache) {
            if ((this._materialCache[m] instanceof THREE.MeshPhysicalMaterial || this._materialCache[m] instanceof THREE.MeshStandardMaterial)) {
                ...
                for (let d in this._materialCache[m].defines) {
                    if (d.startsWith('ENVMAP_TYPE_'))
                        delete this._materialCache[m].defines[d];
                }
                this._materialCache[m].defines['ENVMAP_TYPE_' + this._envMapType.toUpperCase()] = '';
            }
        }
    }
    assignPointSize(p) {
        ...
        for (let m in this._materialCache) {
            if (this._materialCache[m] instanceof THREE.MeshPhysicalMaterial || this._materialCache[m] instanceof THREE.MeshStandardMaterial) {
                if (this._materialCache[m].emissiveMap)
                    this._materialCache[m].emissiveMap.encoding = this._textureEncoding;
                if (this._materialCache[m].map)
                    this._materialCache[m].map.encoding = this._textureEncoding;
                this._materialCache[m].needsUpdate = true;
            }
        }
    }
    ...
    getMaterialProperties(materialData, type, materialSettings) {
        ...
        throw this._logger.handleError(viewer_shared_services_1.LOGGINGTOPIC.DATA_PROCESSING, `MaterialLoader.getMaterialProperties`, error);
    }
    /**
     ...
     */
    load(materialData, materialSettings) {
